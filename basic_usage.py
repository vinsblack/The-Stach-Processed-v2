...
def generate_objective_stats(train_data):
    ...
    return {
        ...
        'quality_stats': {
            'mean': float(np.mean(quality_scores)) if 'quality_score' in train_data[0] else None,
            'median': float(np.median(quality_scores)) if 'quality_score' in train_data[0] else None,
            'std': float(np.std(quality_scores)) if 'quality_score' in train_data[0] else None,
            'high_quality_count': high_quality if 'quality_score' in train_data[0] else None,
            'high_quality_pct': float(high_quality/len(train_data)*100) if 'quality_score' in train_data[0] else None
        },
        'content_stats': {
            'avg_chars': float(np.mean(content_lengths)),
            'avg_lines': float(np.mean(line_counts)),
            'min_chars': int(min(content_lengths)),
            'max_chars': int(max(content_lengths))
        }
    }
